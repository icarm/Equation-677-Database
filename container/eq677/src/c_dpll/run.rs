...
fn main_branch(ctxt: &mut Ctxt) {
    let Some((x, y)) = select_p(ctxt) else {
        submit_model(ctxt);
        return main_backtrack(ctxt);
    };

    defresh(x, ctxt);
    defresh(y, ctxt);

    if branch_options(x, y, 0, ctxt).is_ok() { return main_propagate(ctxt); }
    else { return main_backtrack(ctxt); }
}
    ...
    loop {
        ...
        match event {
            TrailEvent::Decision(x, y, e) => {
                ctxt.chosen_per_row[x as usize] -= 1;
                if branch_options(x, y, e+1, ctxt).is_ok() { return main_propagate(ctxt); }
            },
...
pub fn main_propagate(ctxt: &mut Ctxt) {
    match propagate(ctxt) {
        Ok(()) => return main_branch(ctxt),
        Err(()) => return main_backtrack(ctxt),
    }
}
